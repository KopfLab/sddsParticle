...
You can restrict the app to a subset of devices and set a display
timezone:
...
Key module pieces: `sdds_header()`, `sdds_ui_devices_card()`,
`sdds_ui_structures_card()`, `sdds_server()`, `sdds_onstart()`, and
`sdds_ui_quick_action()`. Finer-grained building blocks
(`sdds_ui_devices_table()`, `sdds_ui_structures_actions()`, individual
device buttons, …) are also exported for custom layouts.
